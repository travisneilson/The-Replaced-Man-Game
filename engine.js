...
<svg width="128" height="48" viewBox="0 0 128 48" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  const Input = {
    ...
    init() {
      window.addEventListener('keydown', e => {
        ...
        // Hint (string or array)
        const hk = Config.KEY.hint;
        const hintMatch = Array.isArray(hk)
          ? hk.includes(k.toLowerCase())
          : k.toLowerCase() === hk;
        if (hintMatch && this.onHint) {
          this.onHint();
        }
        // Interact (string or array)
  ...
  const Game = {
    ...
    async loadRoom(key) {
      ...
      // Prepare nodesData (initial positions)
      ...
      // Clamp positions so nothing leaves the game area
      // AFTER your repel loop, before render…
        nodesData.forEach(nd => {
            const { node } = nd;
            if (node.type === 'exit') {
                // pick horizontal half-extent based on orientation
                const isVertical = node.orientation === 'north' || node.orientation === 'south';
                const halfX = isVertical
                ? Config.DOOR.width  / 2  // north/south: width is horizontal span
                : Config.DOOR.height / 2; // east/west: height becomes horizontal span
                const halfY = isVertical
                ? Config.DOOR.height / 2  // north/south: height is vertical span
                : Config.DOOR.width  / 2; // east/west: width becomes vertical span

                const minX = Config.DOOR.inset + halfX;
                const maxX = MAP_W - (Config.DOOR.inset + halfX);
                const minY = Config.DOOR.inset + halfY;
                const maxY = MAP_H - (Config.DOOR.inset + halfY);

                nd.x = Math.min(Math.max(nd.x, minX), maxX);
                nd.y = Math.min(Math.max(nd.y, minY), maxY);
            } else {
                // …unchanged for other icons…
            }
        });
    ...
    findEntryForKey(key) {
      return this.interactiveNodes.find(entry => {
        ...
        return entry.node.interactions.some(i => i.key.toLowerCase() === key);
      });
    },
    ...
    handleInteract(key) {
      ...
      if (entry.node.type === 'exit') {
        ...
        this.executeAction(interaction.action);
    ...
    loop(now = performance.now()) {
      ...
      if (UI.refs.modal.classList.contains('hidden')) {
        ...
        const hw = UI.refs.player.offsetWidth  / 2;
        const hh = UI.refs.player.offsetHeight / 2;
